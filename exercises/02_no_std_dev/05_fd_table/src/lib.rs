...
impl FdTable {
    ...
    pub fn alloc(&mut self, file: Arc<dyn File>) -> usize {
        // TODO
        if let Some(available_position) = self.table.iter().position(|x| x.is_none()) {
            self.table[available_position] = Some(file);
            available_position
        } else {
